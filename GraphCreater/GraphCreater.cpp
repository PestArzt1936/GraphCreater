...
// GraphCreater.cpp: определяет поведение классов для приложения.
//
...
BEGIN_MESSAGE_MAP(CGraphCreaterApp, CWinApp)
	ON_COMMAND(ID_APP_ABOUT, &CGraphCreaterApp::OnAppAbout)
...
// Создание CGraphCreaterApp

CGraphCreaterApp::CGraphCreaterApp() noexcept
{
	...
	SetAppID(_T("GraphCreater.AppID.NoVersion"));

	// TODO: добавьте код создания,
	// Размещает весь важный код инициализации в InitInstance
}

// Единственный объект CGraphCreaterApp

CGraphCreaterApp theApp;


// Инициализация CGraphCreaterApp

BOOL CGraphCreaterApp::InitInstance()
{
	...
	pDocTemplate = new CSingleDocTemplate(
		IDR_MAINFRAME,
		RUNTIME_CLASS(CGraphCreaterDoc),
		RUNTIME_CLASS(CMainFrame),       // основное окно рамки SDI
		RUNTIME_CLASS(CGraphCreaterView));
	if (!pDocTemplate)
		return FALSE;
...
int CGraphCreaterApp::ExitInstance()
{
...
void CGraphCreaterApp::OnAppAbout()
{
	CAboutDlg aboutDlg;
	aboutDlg.DoModal();
}
void CGraphCreaterApp::ChoseOfVertical() {
	CFrameWnd* pFrame = (CFrameWnd*)AfxGetMainWnd();
	CGraphCreaterView* pView = (CGraphCreaterView*)pFrame->GetActiveView();
	CGraphCreaterDoc* pDoc = pView->GetDocument();
	pDoc->ChosenType = 'V';
#ifdef _DEBUG
AfxMessageBox(_T("Выбран режим вершины"));

#else
#endif // DEBUG
}
void CGraphCreaterApp::ChoseOfEdge() {
	CFrameWnd* pFrame = (CFrameWnd*)AfxGetMainWnd();
	CGraphCreaterView* pView = (CGraphCreaterView*)pFrame->GetActiveView();
	CGraphCreaterDoc* pDoc = pView->GetDocument();
	pDoc->ChosenType = 'E';
